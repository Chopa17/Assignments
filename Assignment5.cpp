...
#include <iostream> //include I/O operations
...
class Complex{
	...
	public:
		...
		//function to return principal argument [-pi,pi]
		...
		//declare overloaded operators as friends
		Complex operator+(const Complex &c){
			Complex sum(real + c.real, imaginary + c.imaginary); //z1 + z2 = (x1 + x2)+i(y1 + y2)
			return sum;
		};
...
Complex operator-(const Complex &c1, const Complex &c2){
	Complex difference(c1.real - c2.real, c1.imaginary - c2.imaginary); //z1 + z2 = (x1 - x2)+i(y1 - y2)
	return difference;
}
...
int main(){
	...
	Complex product{ a*b };
	Complex divisor{ a / b };
	cout << "a+b = " << sum << endl; //addition
	cout << "a-b = " << difference << endl; //subtraction
	cout << "a*b = " << product << endl; //multiplication
	cout << "a/b = " << divisor << endl; //division
